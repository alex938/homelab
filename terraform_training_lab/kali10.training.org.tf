variable "create_k10_vm" {
  type    = bool
  default = true
}

resource "proxmox_vm_qemu" "kali10" {
  count       = var.create_k10_vm ? 1 : 0
  name        = "kali10.training.org"
  desc        = "kali10.training.org"
  vmid        = 1040
  target_node = "d720"

  depends_on = [proxmox_vm_qemu.kali9]
...
EOF
  ipconfig0  = "ip=172.16.1.60/24,gw=172.16.1.1,dns=172.16.1.1"
