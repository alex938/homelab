variable "create_k2_vm" {
  type    = bool
  default = true
}

resource "proxmox_vm_qemu" "kali2" {
  count       = var.create_k2_vm ? 1 : 0
  name        = "kali2.training.org"
  desc        = "kali2.training.org"
  vmid        = 1012
  target_node = "d720"
...
EOF
  ipconfig0  = "ip=172.16.1.52/24,gw=172.16.1.1,dns=172.16.1.1"

  network {
    model  = "virtio"
    bridge = "kali_net"
    macaddr = "02:00:AA:BB:CC:01"
  }
}
